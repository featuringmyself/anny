...
import styles from "@/components/pages/tools/ai-readiness/AiReadinessGauge.module.css";

type AiReadinessGaugeProps = {
  value: number | null;
  label: string;
  pending?: boolean;
};
...
export function AiReadinessGauge({
  ...
  const gradientId = `ar-meter-${useId().replace(/:/g, "")}`;
  ...
  return (
    <svg
      ...
      aria-label={
        pending
          ? "AI readiness meter, scanning"
          : clamped == null
            ? "AI readiness meter, empty"
            : `AI readiness ${label} out of 100`
      }
    >
      <defs>
        <linearGradient id={gradientId} x1="0.15" y1="1" x2="0.85" y2="0">
          ...
          <stop offset="100%" stopColor="#9dffd4" />
        </linearGradient>
      </defs>
      ...
      {pending ? (
        <path
          ...
          d={`M ${CX} ${CY} L ${polar(72, START_DEG).x} ${polar(72, START_DEG).y}`}
          stroke="#7ea1ff"
          strokeWidth="2"
          strokeLinecap="round"
        />
