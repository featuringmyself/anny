import { serpCheckerHowTo } from "@/components/pages/tools/serp-overview/seo";

export function SerpOverviewHowTo() {
  return (
    <section className="border-b" aria-labelledby="serp-howto-heading">
      <div className="border-b px-6 py-10 md:px-12 md:py-14">
        <h2
          id="serp-howto-heading"
          className="text-3xl font-medium tracking-tight md:text-4xl"
        >
          {serpCheckerHowTo.name}
        </h2>
        <p className="mt-3 max-w-xl text-lg text-zinc-500 text-balance">
          {serpCheckerHowTo.description}
        </p>
      </div>
      <ol className="grid md:grid-cols-3">
        {serpCheckerHowTo.steps.map((step, index) => (
          <li
            key={step.name}
